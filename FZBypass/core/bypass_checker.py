...
from FZBypass.core.bypass_truelink import get_truelink_bypass, truelink_resolver
from FZBypass.core.bypass_truelink import TRUELINK_PATTERNS
...
async def direct_link_checker(link, onlylink=False):
    ...
    if bool(match(r"https?:\/\/(yadi|disk.yandex)\.\S+", link)):
        ...
        "1024tera", "terabox", "nephobox", "4funbox",
    ...
    # ---------- FALLBACK: INDIAN SHORTENER ----------
    try:
        fallback = await indian_shortener_bypass(link)
        if fallback:
            blink = fallback
    except DDLException:
        pass

    # ---------- NORMAL ELIF CHAIN CONTINUES ----------
    if blink is None:
        if bool(match(r"https?:\/\/ouo\.\S+", link)):
